...
export const setName = async (payload, meta) => {
  ...
  await sendToAll(Object.keys(NAMES_DB), {
    systemMessage: `${NAMES_DB[meta.connectionId]} has joined the chat`,
  });
  return {};
};
...
export const sendPublic = async (payload, meta) => {
  await sendToAll(Object.keys(NAMES_DB), {
    publicMessage: `${NAMES_DB[meta.connectionId]}: ${payload.message}`,
  });
  return {};
};
...
export const sendPrivate = async (payload, meta) => {
  ...
  await sendToOne(to, {
    privateMessage: `${NAMES_DB[meta.connectionId]}: ${payload.message}`,
  });
  return {};
};
...
export const $disconnect = async (payload, meta) => {
  await sendToAll(Object.keys(NAMES_DB), {
    systemMessage: `${NAMES_DB[meta.connectionId]} has left the chat`,
  });

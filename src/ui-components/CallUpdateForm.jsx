...
export default function CallUpdateForm(props) {
  const {
    id: idProp,
    call: callModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = callRecord
      ? { ...initialValues, ...callRecord }
      : initialValues;
    setARN(cleanValues.ARN);
    setPhone(cleanValues.phone);
    setCallStart(cleanValues.callStart);
    setCallEnd(cleanValues.callEnd);
    setErrors({});
  };
  const [callRecord, setCallRecord] = React.useState(callModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getCall.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCall
        : callModelProp;
    ...
    queryData();
  }, [idProp, callModelProp]);
  React.useEffect(resetStateValues, [callRecord]);
  const validations = {
    ARN: [{ type: "Required" }],
    phone: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateCall.replaceAll("__typename", ""),
            variables: {
              input: {
                id: callRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "CallUpdateForm")}
      {...rest}
    >
      <TextField
        label="Arn"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || callModelProp)}

...
export default function AIChatBox({ open, onClose }: AIChatBoxProps) {
  ...
  const currentMousePosition = useRef<{ top: number; left: number } | null>(
    null
  );
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const onMouseMove = (event: MouseEvent) => {
      ...
      newLeft = Math.max(
        0,
        Math.min(newLeft, window.innerWidth - chatBoxRef.current.offsetWidth)
      );
      newTop = Math.max(
        0,
        Math.min(newTop, window.innerHeight - chatBoxRef.current.offsetHeight)
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/chatBot?question=${encodeURIComponent(input)}`
      );
      if (!response.ok) {
  ...
  return (
    <div
      ...
      style={{
        top:
          open && currentMousePosition.current
            ? currentMousePosition.current.top
            : "initial",
        left:
          open && currentMousePosition.current
            ? currentMousePosition.current.left
            : "initial",
      }}
    >
      <div className="flex flex-col rounded-xl bg-background border shadow-2xl">
        ...
        {!error && messages.length === 0 && (
          <div className="flex h-full items-center justify-center">
            ...
            <span className="loadingNoStyle">...</span>
          </div>
        )}

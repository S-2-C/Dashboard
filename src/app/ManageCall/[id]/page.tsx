...
export default function ManageCall({params} : {params: {id: string}}) {
    ...
    useEffect(() => {
        async function fetchContacts() {
            const res = await fetchContactData(params.id);
        ...
        <div className="flex h-screen bg-background text-foreground relative">
            ...
            <div className="flex flex-col flex-1 p-10 ml-20">
                <Flex direction="column" gap="2rem">
                    ...
                    <Heading level={3} fontWeight="bold">{call?.user?.id} - Walmart®.com</Heading>
                    {call?.user?.role == "SUPERVISOR"?
                        <Text fontWeight="bold">You are a Supervisor</Text>
                        :
                        <Text fontWeight="bold">You are an Agent</Text>
                    }
